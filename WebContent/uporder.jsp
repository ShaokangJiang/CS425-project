...
<br></br>
 ...
 <form action = "uporderCh.jsp" method = "post" onsubmit = "return validate()">  
<div class="row">
        ...
        <div class="col s10 m8 l8">
          <div class="card white z-depth-3">
            <div class="card-content black-text">
              <span class="card-title"><i class="material-icons prefix">find_in_page</i>pick the one you want to modify</span>
      ...
      <label for="myInput">Search for Customer info...</label>
      </div>
...
Select
        ...
        <% 
        PreparedStatement pStmt3 = conn.prepareStatement("select Order_id,Category_Type,Customer_ID,First_Name,Last_Name,Results from dbo.Order_ join dbo.Diagnostic on dbo.Order_.Diagnostic_ID=dbo.Diagnostic.ID join dbo.Patient on dbo.Order_.Customer_ID=dbo.Patient.ID");  
 	   ResultSet rs3 = pStmt3.executeQuery();
		int hs=0;
    	  while(rs3.next()){
    		  hs++;
    		  if(rs3.getString("Results").charAt(0)=='0'){
    %>
          
          <tr>
            <td>
            <p>
      <input class="with-gap" name="group1" type="radio" id="test<%=hs %>" value=<%=rs3.getInt(1) %> />
      <label for="test<%=hs %>">Order ID: <%=rs3.getInt(1) %></label>
    </p>
        
        
</td>
            <td><%="ID:"+rs3.getString(3)+" "+rs3.getString(4)+" "+rs3.getString(5) %></td>
  ...
  <a class="waves-effect waves-light btn blue" href="medicalStaff.jsp?id=<%=id %>"><i class="material-icons prefix">arrow_back</i>Go back</a>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <button class="waves-effect waves-light btn" id="submit"><i class="material-icons prefix">arrow_forward</i>Do some change</button>
