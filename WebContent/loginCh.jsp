...
<html>  
  ...
  <body>  
    <%      
            ...
            if(rs1.next()){
            	//FIXIT: renew the login time here
            	PreparedStatement tmt2 = conn.prepareStatement("update dbo.login set LoginTime='"+ date +"' where id='"+id+"'");
            	int rst1 = tmt2.executeUpdate();  
           	 	if (rst1 != 0){
            	String Privilege1 = rs1.getString("Privilege");
            	if(Privilege1.equalsIgnoreCase("patient")){
            		 rs1.close();
                     pStmt1.close();
                     conn.close();
                     	%>
                	...
                	<%
            	}else if(Privilege1.equalsIgnoreCase("scheduler")){
            		 rs1.close();
                     pStmt1.close();
                     conn.close();
            		%>
            		<jsp:forward page="scheduler.jsp">
                 	<jsp:param name="id" value="<%=id %>"/> 
                 	</jsp:forward>
            		<% 
            	}else if(Privilege1.equalsIgnoreCase("Medical Staff")){
            		 rs1.close();
                     pStmt1.close();
                     conn.close();
            		%>
            		...
            		<% 
            	}else if(Privilege1.equalsIgnoreCase("admin")){
            		 rs1.close();
                     pStmt1.close();
                     conn.close();
            		%>
