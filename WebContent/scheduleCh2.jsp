...
<%    
  ...
  String string=date+" "+stime;
  java.sql.Timestamp stime_ = convert.normalizedate(string);
  string=date+" "+etime;
  java.sql.Timestamp etime_ = convert.normalizedate(string);
  h="The time range you input has been normalized to " + stime_.toString() + " to " + etime_.toString();
...
<br></br>
 ...
 <form action = "scheduleCh3.jsp" method = "post" onsubmit = "return validate()">  
<div class="row">
      ...
      <div class="card-action">
        ...
        <i class="material-icons prefix">event_available</i>Doctor is avaliable at the following time:
...
Select
        ...
        <% 
        PreparedStatement pStmt3 = conn.prepareStatement("select * from dbo.calendar where ID = '" + doc_id + "' and Reserved between '"+stime_ + "' and '"+ etime_ +"'");  
 	   ResultSet rs3 = pStmt3.executeQuery();
		/* PreparedStatement pStmt3 = conn.prepareStatement("select * from dbo.calendar where id = '" + "1" + "'");  
        ResultSet rs3 = pStmt3.executeQuery();
        rs3.next();
        Timestamp a = rs3.getTimestamp(2);
        */ 
       //Assume 30 minutes as one pickup unit
        
       int hs=0;
        ArrayList<java.sql.Timestamp> re=new ArrayList<java.sql.Timestamp>();
        while(rs3.next()){
        	re.add(rs3.getTimestamp(2));
        }
       for(java.sql.Timestamp i = stime_;i.before(etime_);){
    	   boolean g = false;
    	   for(int k=0;k<re.size();k++){
    		   if(re.get(k).equals(i))
    		   {
    			   g = true;
    			   break;
    		   }
    	   ...
    	   if(!g){
    		   Calendar cal = Calendar.getInstance();
    	          cal.setTimeInMillis(i.getTime());
    	          cal.add(Calendar.MINUTE, 30);
    	          java.sql.Timestamp late = new Timestamp(cal.getTime().getTime());
    	          hs++;
    	          String[] ab = i.toString().split(" ");
    	          String hd = ab[0]+";"+ab[1];
    ...
    %>
          ...
          <tr>
            <td>
            <p>
      <input class="with-gap" name="group1" type="radio" id="test<%=hs %>" value=<%=hd %> />
      <label for="test<%=hs %>">Slot <%=hs %></label>
    </p>
          ...
          cal.setTimeInMillis(i.getTime());
          cal.add(Calendar.MINUTE, 30);
          i = new Timestamp(cal.getTime().getTime());   
  ...
  <a class="waves-effect waves-light btn blue" href="scheduleCh.jsp?id=<%=id %>"><i class="material-icons prefix">arrow_back</i>Go back</a>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
