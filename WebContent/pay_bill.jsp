...
<br></br>
    ...
    <form action = "payCh.jsp" method = "post" onsubmit = "return validate()">  
<div class="row">
      ...
      <label for="myInput">Search for name...</label>
      </div>

          <table id="myTable"  class="highlight sortable">
    <thead>
    <tr>
              <th>
Select
        ...
        <% 
        PreparedStatement pStmt3 = conn.prepareStatement("select Order_id,First_Name,Last_Name,Category_Type,Results,Cost From dbo.Order_ join dbo.Diagnostic on dbo.Order_.Diagnostic_ID = dbo.Diagnostic.ID join dbo.Patient on dbo.Order_.Customer_ID = dbo.Patient.ID");  
 	    ResultSet rs3 = pStmt3.executeQuery();
    	int hs = 0;  
 	    while(rs3.next()){
    		  if(rs3.getString(5).charAt(0)=='1' && rs3.getString(5).charAt(1)=='0') {
    			hs++;
    %>
          <tr>
            <td>
...
function myFunction() {
  ...
  <a class="waves-effect waves-light btn blue" href="admin.jsp?id=<%=id %>"><i class="material-icons prefix">arrow_back</i>Go back</a>
    <button class="waves-effect waves-light btn" id="submit">Pay</button>
